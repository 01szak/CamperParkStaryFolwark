...
public interface GuestRepository extends CRUDRepository<Guest> {
    ...
    @Query("""
        SELECT c, cnt FROM (
            SELECT g.country c, count(*) cnt FROM 
                Guest g JOIN Reservation r ON r.id = g.id 
                    WHERE :month = 0 OR FUNCTION('MONTH', r.checkin) = :month AND :year = 0 OR FUNCTION('YEAR', r.checkin) = :year)
